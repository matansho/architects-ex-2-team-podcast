...
def search_cascade(
    ...
    *,
    ...
    for path, _score in file_hits:
        candidate_idxs.extend(file_bm25.file_to_idxs.get(path) or [])
...
def search_rrf(
    ...
    *,
    ...
    index = id_to_idx if id_to_idx is not None else build_id_index(vectors)
    dense_hits = search(query_emb, corpus_emb, vectors, top_k=candidate_n)
    ...
    bm25_idxs = [i for i, _ in bm25_hits]
...
def search_reranked(
    ...
    *,
    ...
    """
    index = id_to_idx if id_to_idx is not None else build_id_index(vectors)
    candidates = search(query_emb, corpus_emb, vectors, top_k=candidate_n)
    if not candidates:
