#!/usr/bin/env bash
# Full Train-48Q with query decomposition, judged by Kimi-K3.
# Answer prompt is the baseline (v6 reverted). Only --decompose is new vs the
...
ANSWERS=reports/rag_answers_routectx_kimik3_low_decompose_48q.jsonl
EVAL=reports/stage2/rag_routectx_kimik3_low_decompose_48q_corpus_judge_eval.json
...
echo "=== GEN: 48Q routectx + Kimi-K3 low + decompose === $(date +%T)"
...
.venv/bin/python run_eval.py \
  ...
  --label routectx-kimik3-low-decompose-48q \
  --out "$EVAL"
